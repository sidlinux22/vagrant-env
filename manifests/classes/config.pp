# replacefile::config
#
# This resource is an example of what you can do with replacefile.
# replacefile::config manages simple configuration files which follow the scheme
# PARAMETER = VALUE
...
define replacefile::config( $path, $entry = false, $ensure, $sep = '=', $quote = false, $no_fail_without_parent = false ) {
  ...
  if $entry == false {
    ...
    if $ensure == absent {
      ...
      replacefile { "${title}":
        ...
        # OR
        # if there is no match, replacefile automatically appends our entry at EOF
