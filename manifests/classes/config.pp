# efile::config
#
# This resource is an example of what you can do with efile.
# efile::config manages simple configuration files which follow the scheme
# PARAMETER = VALUE
...
define efile::config( $path, $entry = false, $ensure, $sep = '=', $quote = false, $no_fail_without_parent = false ) {
  ...
  if $entry == false {
    ...
    if $ensure == absent {
      ...
      efile { "${title}":
        ...
        # OR
        # if there is no match, efile automatically appends our entry at EOF
